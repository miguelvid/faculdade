...
int main() {
    ...
    eventos[evento_count++] = (evento){entidades[0].TC, 1, cont++, 'c'};
    eventos[evento_count++] = (evento){entidades[0].TS, 1, cont--, 's'};
    ...
    for(i = 1; i < qtd; i++) {
        ...
        eventos[evento_count++] = (evento){entidades[i].TC, i + 1, cont ++, 'c'};
        ...
        eventos[evento_count++] = (evento){entidades[i].TS, i + 1, cont--, 's'};
    }
    ...
    for(i = 0; i < evento_count; i++) {
        printf("%3d | %3d |  %c | %d\n", eventos[i].TEV, eventos[i].ENT, eventos[i].TpEV, eventos[i].NrEnt);
    }
